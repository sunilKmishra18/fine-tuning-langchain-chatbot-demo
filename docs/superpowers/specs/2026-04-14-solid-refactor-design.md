...
**Date:** 2026-04-14
**Scope:** Refactor `app.py` into multiple focused modules applying all five SOLID principles.

---

## 1. Goal

Split the single `app.py` script into five focused modules. Each module has one reason to change. The LLM provider is abstracted behind a `ChatModel` protocol so future providers can be swapped in without touching chat or dataset logic.
